...
router.get("/:id", (req, res, next) => {
  ...
  book.findById(id, function (err: any, book: any) {
    if (err) {
      console.error(err);
      res.end(err);
    }
...
router.post("/:id", (req, res, next) => {
  ...
  // console.log(editedBook._id);

  book.updateOne({ _id: id }, editedBook, function (err: CallbackError) {
    if (err) {
      console.error(err);
      res.end(err);
    }
    // After updated then go back to Book List page
    res.redirect("/books");
  });
});
...
router.get("/delete/:id", (req, res, next) => {
  ...
  book.remove({ _id: id }, function (err: CallbackError) {
    ...
    // If delete successfully, then go back to Book List page
    res.redirect("/books");
  });
});
